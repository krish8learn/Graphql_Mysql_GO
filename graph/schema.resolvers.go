...
func (r *mutationResolver) CreateUser(ctx context.Context, input model.NewUser) (string, error) {
	...
	user.Create()
	token, err := jwtLogin.GenerateToken(user.Username)
	if err != nil {
		return "", err
	}
	return token, nil
...
func (r *mutationResolver) RefreshToken(ctx context.Context, input model.RefreshTokenInput) (string, error) {
	username, err := jwtLogin.ParseToken(input.Token)
	if err != nil {
		return "", fmt.Errorf("Access Denied")
	}

	token, err := jwtLogin.GenerateToken(username)
	if err != nil {
		return "", err
	}
	return token, nil
